...
LOG_FILE="/tmp/flutter-setup-$(date +%Y%m%d-%H%M%S).log"
...
validate_project_name() {
    ...
    if [[ ! "$name" =~ ^[a-z][a-z0-9_]*$ ]]; then
        echo "Error: Project name must start with a letter and contain only lowercase letters, numbers, and underscores"
        return 1
    fi
...
copy_template() {
    ...
    for dir in android assets fonts lib test; do
...
customize_template() {
    ...
    if [ -f "$dest_dir/pubspec.yaml" ]; then
        sed -i.bak "s/flutter_template_app/$project_name/g" "$dest_dir/pubspec.yaml"
        rm -f "$dest_dir/pubspec.yaml.bak"
    fi

    if [ -f "$dest_dir/android/app/build.gradle" ]; then
        sed -i.bak "s/flutter_template_app/$project_name/g" "$dest_dir/android/app/build.gradle"
        rm -f "$dest_dir/android/app/build.gradle.bak"
    fi
}
...
main() {
    ...
    echo "Run: cd $PROJECT_DIR && flutter run"
}
