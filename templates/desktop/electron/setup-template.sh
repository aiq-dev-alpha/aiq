...
LOG_FILE="/tmp/electron-setup-$(date +%Y%m%d-%H%M%S).log"
...
customize_template() {
    ...
    find "$dest_dir" -name "*.js" -type f -exec sed -i.bak "s/{{PROJECT_NAME}}/$project_name/g" {} \;
    find "$dest_dir" -name "*.html" -type f -exec sed -i.bak "s/{{PROJECT_NAME}}/$project_name/g" {} \;
