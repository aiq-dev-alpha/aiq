...
copy_template() {
    ...
    echo "Setting up Tauri project: $project_name"

    mkdir -p "$dest_dir"

    find "$TEMPLATE_DIR" -maxdepth 1 -type f ! -name "*.sh" -exec cp {} "$dest_dir/" \;

    for dir in src src-tauri; do
...
customize_template() {
    ...
    if [ -f "$dest_dir/package.json" ]; then
        sed -i.bak "s/tauri-template/$project_name/g" "$dest_dir/package.json"
        rm -f "$dest_dir/package.json.bak"
    fi

    if [ -f "$dest_dir/src-tauri/Cargo.toml" ]; then
        sed -i.bak "s/tauri-template/$project_name/g" "$dest_dir/src-tauri/Cargo.toml"
        rm -f "$dest_dir/src-tauri/Cargo.toml.bak"
    fi

    if [ -f "$dest_dir/src-tauri/tauri.conf.json" ]; then
        sed -i.bak "s/Tauri Template/$project_name/g" "$dest_dir/src-tauri/tauri.conf.json"
        sed -i.bak "s/com.tauri.template/com.$project_name.app/g" "$dest_dir/src-tauri/tauri.conf.json"
        rm -f "$dest_dir/src-tauri/tauri.conf.json.bak"
    fi

    if [ -f "$dest_dir/index.html" ]; then
        sed -i.bak "s/Tauri + React + TypeScript/$project_name/g" "$dest_dir/index.html"
...
main() {
    if [ $# -ne 2 ]; then
        ...
        echo "Example: $0 my-tauri-app ~/projects/"
        exit 1
    fi
