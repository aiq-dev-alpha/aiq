...
copy_template() {
    ...
    echo "Setting up Svelte TypeScript project: $project_name"

    mkdir -p "$dest_dir"

    find "$TEMPLATE_DIR" -maxdepth 1 -type f ! -name "*.sh" -exec cp {} "$dest_dir/" \;

    for dir in src static; do
...
customize_template() {
    ...
    if [ -f "$dest_dir/package.json" ]; then
        sed -i.bak "s/svelte-typescript-template/$project_name/g" "$dest_dir/package.json"
        rm -f "$dest_dir/package.json.bak"
    fi

    if [ -f "$dest_dir/src/app.html" ]; then
        sed -i.bak "s/Svelte TypeScript Template/$project_name/g" "$dest_dir/src/app.html"
        rm -f "$dest_dir/src/app.html.bak"
    fi

    find "$dest_dir/src" -type f -name "*.svelte" -exec sed -i.bak "s/Svelte TypeScript Template/$project_name/g" {} \;
...
main() {
    if [ $# -ne 2 ]; then
        ...
        echo "Example: $0 my-svelte-app ~/projects/"
        exit 1
    fi
