...
copy_template() {
    ...
    echo "Setting up Vue 3 TypeScript project: $project_name"
...
customize_template() {
    ...
    if [ -f "$dest_dir/package.json" ]; then
        sed -i.bak "s/vue3-typescript-template/$project_name/g" "$dest_dir/package.json"
        rm -f "$dest_dir/package.json.bak"
    fi

    if [ -f "$dest_dir/index.html" ]; then
        sed -i.bak "s/Vue 3 TypeScript Template/$project_name/g" "$dest_dir/index.html"
        rm -f "$dest_dir/index.html.bak"
    fi

    find "$dest_dir/src" -type f -name "*.vue" -exec sed -i.bak "s/Vue 3 TypeScript Template/$project_name/g" {} \;
...
main() {
    if [ $# -ne 2 ]; then
        ...
        echo "Example: $0 my-vue-app ~/projects/"
        exit 1
    fi
