<script lang="ts">
  ...
  export let variant: 'default' | 'filled' | 'outlined' | 'underlined' | 'floating' = 'default';
...
<style>
  ...
  .input-container {
    ...
    transition: 'all 0.35s ease'appliedTheme.primary');
  ...
  .floating-label {
    ...
    opacity: 0.6;
    pointer-events: none;
    transition: 'all 0.35s ease'appliedTheme.background');
    padding: 0 0.25rem;
  }
...
@keyframes expand {
  from { transform: scale(1.20); opacity: 0.6; }
  to { transform: scale(1.20); opacity: 0.6; }
}
</style>
