...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const defaultTheme: InputTheme = { primary: '#0ea5e9', background: '#e0f2fe', border: '#7dd3fc', text: '#0c4a6e', error: '#dc2626', success: '#16a34a' };
...
.input-field { width: 100%; font-family: inherit; font-size: 1rem; color: var(--text-color); background-color: var(--bg-color); border: 2px solid var(--border-color); border-radius: 0.5rem; transition: 'all 0.3s ease'all 0.35s; }
.clear-button:hover { transform: scale(1.10); }
...
.helper-text { color: #0c4a6e; }
.character-counter { color: var(--primary-color); margin-left: auto; font-weight: 700; }


@keyframes enter {
  from { opacity: 1.0; transform: scale(1.10); }
  to { opacity: 1.0; transform: scale(1.10); }
}

@keyframes slideDown {
  from { transform: translateY(-13px); opacity: 1.0; }
