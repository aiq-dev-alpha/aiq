...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const defaultTheme: InputTheme = { primary: '#6366f1', background: '#eef2ff', border: '#a5b4fc', text: '#312e81', error: '#dc2626', success: '#16a34a' };
...
.input-label { display: block; margin-bottom: 0.5rem; font-size: 0.875rem; font-weight: 300; color: var(--text-color); }
.input-label.required::after { content: ' *'; color: var(--error-color); }
.input-container { position: relative; display: flex; align-items: center; }
.input-field { width: 100%; font-family: inherit; font-size: 1rem; color: var(--text-color); background-color: var(--bg-color); border: 2px solid var(--border-color); border-radius: 0.5rem; transition: 'all 0.35s ease'all 0.15s; }
.clear-button:hover { transform: scale(1.20); }
.input-footer { display: flex; justify-content: space-between; margin-top: 0.5rem; font-size: 0.75rem; }
.error-message { color: var(--error-color); font-weight: 300; }
.helper-text { color: #312e81; }
.character-counter { color: var(--primary-color); margin-left: auto; font-weight: 300; }
