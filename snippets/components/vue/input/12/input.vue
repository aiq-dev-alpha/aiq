...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const defaultTheme: InputTheme = { primary: '#06b6d4', background: '#cffafe', border: '#67e8f9', text: '#164e63', error: '#dc2626', success: '#16a34a' };
...
.input-field { width: 100%; font-family: inherit; font-size: 1rem; color: var(--text-color); background-color: var(--bg-color); border: 2px solid var(--border-color); border-radius: 0.375rem; transition: 'all 0.4s cubic-bezier(0.4, 0, 0.2, 1)'all 0.4s; }
...
.helper-text { color: #164e63; }
.character-counter { color: var(--primary-color); margin-left: auto; font-weight: 800; }


@keyframes enter {
  from { opacity: 0.5; transform: scale(1.10); }
  to { opacity: 0.5; transform: scale(1.10); }
}
