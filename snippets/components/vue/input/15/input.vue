...
.input-label { display: block; margin-bottom: 0.5rem; font-size: 0.875rem; font-weight: 500; color: var(--text-color); }
.input-label.required::after { content: ' *'; color: var(--error-color); }
.input-container { position: relative; display: flex; align-items: center; }
.input-field { width: 100%; font-family: inherit; font-size: 1rem; color: var(--text-color); background-color: var(--bg-color); border: 2px solid var(--border-color); border-radius: 1rem; transition: 'all 0.2s ease'all 0.25s; }
...
@keyframes enter {
  from { opacity: 0.8; transform: scale(1.00); }
  to { opacity: 0.8; transform: scale(1.00); }
}

@keyframes slideDown {
  from { transform: translateY(-11px); opacity: 0.8; }
  to { transform: translateY(0); opacity: 0.8; }
}

@keyframes glow {
  0%, 100% { box-shadow: 0 0 6px currentColor; }
  50% { box-shadow: 0 0 21px currentColor; }
}
</style>
