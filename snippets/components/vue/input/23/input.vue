...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const defaultTheme: InputTheme = { primary: '#fb923c', background: '#ffedd5', border: '#fed7aa', text: '#7c2d12', error: '#dc2626', success: '#16a34a' };
...
.input-label { display: block; margin-bottom: 0.5rem; font-size: 0.875rem; font-weight: 700; color: var(--text-color); }
.input-label.required::after { content: ' *'; color: var(--error-color); }
.input-container { position: relative; display: flex; align-items: center; }
.input-field { width: 100%; font-family: inherit; font-size: 1rem; color: var(--text-color); background-color: var(--bg-color); border: 2px solid var(--border-color); border-radius: 0.5rem; transition: 'all 0.2s ease'all 0.35s; }
...
.helper-text { color: #7c2d12; }
.character-counter { color: var(--primary-color); margin-left: auto; font-weight: 700; }


@keyframes enter {
  from { opacity: 1.0; transform: scale(1.15); }
  to { opacity: 1.0; transform: scale(1.15); }
}

@keyframes slideDown {
  from { transform: translateY(-12px); opacity: 1.0; }
