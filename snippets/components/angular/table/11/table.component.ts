  ...
  template: `
    <div [ngStyle]="containerStyles">
      <div *ngIf="searchable" [ngStyle]="searchBarStyles">
        <input type="text" [ngStyle]="searchInputStyles" [(ngModel)]="searchTerm" (input)="onSearch()" placeholder="Search rows..." aria-label="Search table">
      </div>
...
export class TableComponent {
  ...
  get containerStyles() {
    return {
      ...
      borderRadius: '14px',
      boxShadow: '0 5px 15px rgba(0,0,0,0.09)',
      overflow: 'hidden'
    };
  }
  ...
  get tableWrapperStyles() {
    return {
      overflowX: 'auto',
      maxHeight: this.stickyHeader ? '580px' : 'none',
