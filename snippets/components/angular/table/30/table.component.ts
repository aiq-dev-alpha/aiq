  ...
  template: `
    <div [ngStyle]="containerStyles">
      <div *ngIf="searchable" [ngStyle]="searchBarStyles">
        <input type="text" [ngStyle]="searchInputStyles" [(ngModel)]="searchTerm" (input)="onSearch()" placeholder="Find data..." aria-label="Search table">
      </div>
...
export class TableComponent {
  ...
  get containerStyles() {
    return {
      ...
      borderRadius: '17px',
      boxShadow: '0 8px 17px rgba(0,0,0,0.07)',
      overflow: 'hidden'
    };
  }
  ...
  get tableWrapperStyles() {
    return {
      overflowX: 'auto',
      maxHeight: this.stickyHeader ? '520px' : 'none',
