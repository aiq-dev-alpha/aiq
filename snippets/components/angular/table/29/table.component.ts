  ...
  template: `
    <div [ngStyle]="containerStyles">
      <div *ngIf="searchable" [ngStyle]="searchBarStyles">
        <input type="text" [ngStyle]="searchInputStyles" [(ngModel)]="searchTerm" (input)="onSearch()" placeholder="Search here..." aria-label="Search table">
      </div>
...
export class TableComponent {
  ...
  get containerStyles() {
    return {
      ...
      borderRadius: '11px',
      boxShadow: '0 5px 11px rgba(0,0,0,0.09)',
      overflow: 'hidden'
    };
  }
  ...
  get tableWrapperStyles() {
    return {
      overflowX: 'auto',
      maxHeight: this.stickyHeader ? '640px' : 'none',
