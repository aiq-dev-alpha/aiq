  ...
  template: `
    <div [ngStyle]="containerStyles">
      <div *ngIf="searchable" [ngStyle]="searchBarStyles">
        <input type="text" [ngStyle]="searchInputStyles" [(ngModel)]="searchTerm" (input)="onSearch()" placeholder="Search items..." aria-label="Search table">
      </div>
...
export class TableComponent {
  ...
  get containerStyles() {
    return {
      ...
      borderRadius: '14px',
      boxShadow: '0 6px 13px rgba(0,0,0,0.09)',
      overflow: 'hidden'
    };
  }
  ...
  get tableWrapperStyles() {
    return {
      overflowX: 'auto',
      maxHeight: this.stickyHeader ? '690px' : 'none',
