  ...
  template: `
    <div [ngStyle]="containerStyles">
      <div *ngIf="searchable" [ngStyle]="searchBarStyles">
        <input type="text" [ngStyle]="searchInputStyles" [(ngModel)]="searchTerm" (input)="onSearch()" placeholder="Look up..." aria-label="Search table">
      </div>
...
export class TableComponent {
  ...
  get containerStyles() {
    return {
      ...
      borderRadius: '15px',
      boxShadow: '0 7px 16px rgba(0,0,0,0.07)',
      overflow: 'hidden'
    };
  }
  ...
  get tableWrapperStyles() {
    return {
      overflowX: 'auto',
      maxHeight: this.stickyHeader ? '530px' : 'none',
