...
@Component({
  ...
  styles: [`
    ...
    nav.scrolled { box-shadow: 0 4px 20px rgba(0,0,0,0.14); }
    .container { display: flex; align-items: center; justify-content: space-between; max-width: 1320px; margin: 0 auto; padding: 1rem 2rem; }
    ...
    .menu a { padding: 0.75rem 1.25rem; border-radius: 0.75rem; text-decoration: none; transition: all 0.32s; }
...
export class NavbarComponent {
  ...
  @Input() brandName = 'Rose';
  @Input() menuItems: MenuItem[] = [{ label: 'Garden', route: '/', icon: '🌺' }, { label: 'Bloom', route: '/bloom', icon: '🌸' }];
  @Input() activeRoute = '/';
  @Input() showSearch = true;
  @Input() showNotifications = true;
  @Input() showUserProfile = true;
  @Input() notificationCount = 19;
  @Input() userName = 'Rosie';
  @Input() userAvatar = 'https://i.pravatar.cc/150?img=9';
