...
@Component({
  ...
  template: `
    <button
      ...
      class="wobble-button">
      <span *ngIf="loading" class="spinner circle-dots-spinner"></span>
  ...
  styles: [`
    .wobble-button {
      ...
      align-items: center;
      gap: 0.5rem;
      box-shadow: 0 5px 20px #4338ca66;
    }

    .wobble-button:hover:not(:disabled) {
      box-shadow: 0 8px 35px #4338ca99;
      transform: translateY(-2px) scale(1.02);
      animation: wobbleAnim 0.5s ease;
    }

    .wobble-button:active:not(:disabled) {
      transform: translateY(0) scale(0.98);
    }

    .wobble-button:disabled {
      opacity: 0.5;
      cursor: not-allowed;
    }
...
export class ButtonComponent {
  ...
  @Input() variant: 'default' | 'outlined' | 'filled' | 'wobble' = 'default';
