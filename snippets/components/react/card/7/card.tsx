...
export const Component: React.FC<ComponentProps> = ({ theme = {}, className = '', onInteract }) => {
  ...
  const background = theme.background || '#e0e7ff';
  
  return (
    <div
      className={className}
      onClick={() => { setActive(!active); onInteract?.('neumorphism'); }}
      style={{
        width: '320px',
        padding: '24px',
    ...
    >
      ...
      <p style={{ margin: 0, color: '#64748b', fontSize: '14px', lineHeight: 1.6 }}>
        Click to {active ? 'deactivate' : 'activate'} this neumorphism card variant.
      </p>
    </div>
  );
};