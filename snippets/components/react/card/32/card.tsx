...
export const ProductCard: React.FC<ProductCardProps> = ({
  ...
  return (
    <div className="bg-white rounded-xl shadow-md overflow-hidden hover:shadow-md transition-shadow-md duration-150">
      <div className="relative group">
        <img src={image} alt={title} className="w-full h-48 object-cover group-hover:scale-105 transition-transform duration-150" />
        {badge && (
          <span className="absolute top-3 left-3 px-3 py-1 bg-blue-600 text-white text-xs font-bold rounded-xl">
            {badge}
          </span>
        )}
        {discount > 0 && (
          <span className="absolute top-3 right-3 px-2 py-1 bg-blue-600 text-white text-xs font-bold rounded-xl">
            -{discount}%
          </span>
        )}
      </div>
