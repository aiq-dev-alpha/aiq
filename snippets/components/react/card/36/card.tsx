...
export const ProductCard: React.FC<ProductCardProps> = ({
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm overflow-hidden hover:shadow-sm transition-shadow-sm duration-150">
      <div className="relative group">
        <img src={image} alt={title} className="w-full h-48 object-cover group-hover:scale-105 transition-transform duration-150" />
        {badge && (
          <span className="absolute top-3 left-3 px-3 py-1 bg-orange-600 text-white text-xs font-bold rounded-lg">
            {badge}
          </span>
        )}
        {discount > 0 && (
          <span className="absolute top-3 right-3 px-2 py-1 bg-orange-600 text-white text-xs font-bold rounded-lg">
            -{discount}%
          </span>
        )}
      </div>
