...
export const ProfileCard: React.FC<ProfileCardProps> = ({
  ...
  return (
    <div className="bg-white rounded-xl shadow-2xl overflow-hidden max-w-sm">
      ...
      <div className="px-6 pb-6">
        <div className="relative -mt-16 mb-4">
          <img
            src={avatar}
            alt={name}
            className="w-32 h-32 rounded-xl border-4 border-white shadow-2xl object-cover mx-auto"
          />
        </div>
          ...
          <div className="flex justify-center gap-3 mb-4">
            {social.map((item, index) => (
              <a
                key={index}
                href={item.url}
                className="p-2 bg-gray-100 hover:bg-green-100 rounded-xl transition-colors"
                target="_blank"
