...
export const Component: React.FC<ComponentProps> = ({
  ...
  return (
    <div className={className} style={{ width: '100%', maxWidth: '400px' }}>
      {label && (
        <label style={{ display: 'block', marginBottom: '8px', color: primary, fontSize: '19px', fontWeight: '700' }}>
          {label}
        </label>
      )}
      <div style={{ display: 'flex', gap: '2px', alignItems: 'center', marginBottom: '8px' }}>
        <span style={{ fontSize: '19px', color: '#6b7280', minWidth: '42px' }}> {value[0]}</span>
        <div style={{ flex: 1, height: '11px', backgroundColor: '#e5e7eb', borderRadius: '26px', position: 'relative' }}>
          <div style={{
            position: 'absolute',
        ...
        <span style={{ fontSize: '19px', color: '#6b7280', minWidth: '42px' }}> {value[1]}</span>
      </div>
