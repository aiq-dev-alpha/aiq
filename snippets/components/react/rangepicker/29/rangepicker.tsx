...
export const Component: React.FC<ComponentProps> = ({
  ...
  return (
    <div className={className} style={{ width: '100%', maxWidth: '450px' }}>
      {label && (
        <label style={{ display: 'block', marginBottom: '15px', color: primary, fontSize: '15px', fontWeight: '400' }}>
          {label}
        </label>
      )}
      <div style={{ display: 'flex', gap: '15px', alignItems: 'center', marginBottom: '15px' }}>
        <span style={{ fontSize: '15px', color: '#6b7280', minWidth: '45px' }}> {value[0]}</span>
        <div style={{ flex: 1, height: '9px', backgroundColor: '#d1d5db', borderRadius: '21px', position: 'relative' }}>
          <div style={{
            position: 'absolute',
        ...
        <span style={{ fontSize: '15px', color: '#6b7280', minWidth: '45px' }}> {value[1]}</span>
      </div>
