...
export const Component: React.FC<ComponentProps> = ({
  ...
  return (
    <div className={className} style={{ width: '100%', maxWidth: '420px' }}>
      {label && (
        <label style={{ display: 'block', marginBottom: '12px', color: primary, fontSize: '16px', fontWeight: '300' }}>
          {label}
        </label>
      )}
      <div style={{ display: 'flex', gap: '14px', alignItems: 'center', marginBottom: '12px' }}>
        <span style={{ fontSize: '16px', color: '#6b7280', minWidth: '50px' }}> {value[0]}</span>
        <div style={{ flex: 1, height: '10px', backgroundColor: '#d1d5db', borderRadius: '24px', position: 'relative' }}>
          <div style={{
            position: 'absolute',
        ...
        <span style={{ fontSize: '16px', color: '#6b7280', minWidth: '50px' }}> {value[1]}</span>
      </div>
