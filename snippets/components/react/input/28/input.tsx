...
export const FloatingInput: React.FC<FloatingInputProps> = ({
  ...
  return (
    <div className="relative sm:px-4 md:px-6 lg:px-8">
      <div className="relative sm:px-4 md:px-6 lg:px-8">
        {icon && (
          <div className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 sm:px-4 md:px-6 lg:px-8">
            {icon}
          </div>
        )}
        <input
      ...
      {error && (
        <p className="mt-1 text-sm text-orange-600 sm:px-4 md:px-6 lg:px-8">{error}</p>
      )}
      {helperText && !error && (
        <p className="mt-1 text-sm text-gray-500 sm:px-4 md:px-6 lg:px-8">{helperText}</p>
      )}
    </div>
  );
};
