  ...
  return (
    <>
      <style>{`
        ...
        @keyframes popIn {
          0% { opacity: 0; transform: scale(0.5) translateY(50px); } 50% { transform: scale(1.1) translateY(-10px); } 100% { opacity: 1; transform: scale(1) translateY(0); }
        }
        .modal-v18-close-btn:hover {
          transform: scale(1.1) rotate(90deg);
        }
        .modal-v18-primary-btn:hover {
          transform: translateY(-2px);
          filter: brightness(110%);
        }
        .modal-v18-secondary-btn:hover {
          background: #d1d5db;
        }
      `}</style>
      <div style={overlayStyle} onClick={closeOnOverlay ? onClose : undefined}>
        <div ref={modalRef} style={modalStyle} onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby="modal-title">
          ...
          <div style={footerStyle}>
            <button className="modal-v18-secondary-btn" style={secondaryButtonStyle} onClick={onClose}>
              Cancel
            </button>
            <button className="modal-v18-primary-btn" style={primaryButtonStyle} onClick={onClose}>
              Confirm
            </button>
          </div>
        </div>
