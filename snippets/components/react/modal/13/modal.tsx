...
export const Modal: React.FC<ModalProps> = ({
  ...
  const modalStyle: CSSProperties = {
    ...
    maxHeight: '90vh',
    animation: 'flipY 0.5s cubic-bezier(0.34, 1.56, 0.64, 1)',
    position: 'relative'
  };
  ...
  return (
    <>
      <style>{`
        ...
        .modal-v13-close-btn:hover {
          transform: scale(1.1) rotate(90deg);
        }
        .modal-v13-primary-btn:hover {
          transform: translateY(-2px);
          filter: brightness(110%);
        }
        .modal-v13-secondary-btn:hover {
          background: #d1d5db;
        }
      `}</style>
      <div style={overlayStyle} onClick={closeOnOverlay ? onClose : undefined}>
        <div ref={modalRef} style={modalStyle} onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby="modal-title">
          ...
          <div style={footerStyle}>
            <button className="modal-v13-secondary-btn" style={secondaryButtonStyle} onClick={onClose}>
              Cancel
            </button>
            <button className="modal-v13-primary-btn" style={primaryButtonStyle} onClick={onClose}>
              Confirm
            </button>
          </div>
        </div>
