  ...
  return (
    <>
      <style>{`
        ...
        @keyframes fadeScale {
          0% { opacity: 0; transform: scale(1.2); filter: blur(10px); } 100% { opacity: 1; transform: scale(1); filter: blur(0); }
        }
        .modal-v11-close-btn:hover {
          transform: scale(1.1) rotate(90deg);
        }
        .modal-v11-primary-btn:hover {
          transform: translateY(-2px);
          filter: brightness(110%);
        }
        .modal-v11-secondary-btn:hover {
          background: #d1d5db;
        }
      `}</style>
      <div style={overlayStyle} onClick={closeOnOverlay ? onClose : undefined}>
        <div ref={modalRef} style={modalStyle} onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby="modal-title">
          ...
          <div style={footerStyle}>
            <button className="modal-v11-secondary-btn" style={secondaryButtonStyle} onClick={onClose}>
              Cancel
            </button>
            <button className="modal-v11-primary-btn" style={primaryButtonStyle} onClick={onClose}>
              Confirm
            </button>
          </div>
        </div>
