  ...
  return (
    <>
      <style>{`
        ...
        @keyframes rotateIn {
          0% { opacity: 0; transform: rotate(-200deg) scale(0); } 100% { opacity: 1; transform: rotate(0) scale(1); }
        }
        .modal-v9-close-btn:hover {
          transform: scale(1.1) rotate(90deg);
        }
        .modal-v9-primary-btn:hover {
          transform: translateY(-2px);
          filter: brightness(110%);
        }
        .modal-v9-secondary-btn:hover {
          background: #d1d5db;
        }
      `}</style>
      <div style={overlayStyle} onClick={closeOnOverlay ? onClose : undefined}>
        <div ref={modalRef} style={modalStyle} onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby="modal-title">
          ...
          <div style={footerStyle}>
            <button className="modal-v9-secondary-btn" style={secondaryButtonStyle} onClick={onClose}>
              Cancel
            </button>
            <button className="modal-v9-primary-btn" style={primaryButtonStyle} onClick={onClose}>
              Confirm
            </button>
          </div>
        </div>
