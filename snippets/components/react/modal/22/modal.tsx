...
export const Modal: React.FC<ModalProps> = ({
  ...
  const modalStyle: CSSProperties = {
    ...
    maxHeight: '90vh',
    animation: 'rotateScale 0.5s cubic-bezier(0.34, 1.56, 0.64, 1)',
    position: 'relative'
  };
    ...
    <>
      <style>{`
        ...
        @keyframes rotateScale {
          0% { opacity: 0; transform: rotate(180deg) scale(0.5); } 100% { opacity: 1; transform: rotate(0deg) scale(1); }
        }
        .modal-v22-close-btn:hover {
          transform: scale(1.1) rotate(90deg);
        }
        .modal-v22-primary-btn:hover {
          transform: translateY(-2px);
          filter: brightness(110%);
        }
        .modal-v22-secondary-btn:hover {
          background: #d1d5db;
        }
      `}</style>
      <div style={overlayStyle} onClick={closeOnOverlay ? onClose : undefined}>
        <div ref={modalRef} style={modalStyle} onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby="modal-title">
          ...
          <div style={footerStyle}>
            <button className="modal-v22-secondary-btn" style={secondaryButtonStyle} onClick={onClose}>
              Cancel
            </button>
            <button className="modal-v22-primary-btn" style={primaryButtonStyle} onClick={onClose}>
              Confirm
            </button>
          </div>
        </div>
